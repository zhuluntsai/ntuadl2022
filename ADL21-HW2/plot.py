...
model_name = 'squad_2e-5_tv_no_pretrain'
...
def parse_args():
    ...
    parser.add_argument(
        "--train_file", type=str, default='./data/train_valid_squad.json', help="A csv or a json file containing the training data."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--validation_file", type=str, default='./data/valid_squad.json', help="A csv or a json file containing the validation data."
    )
    parser.add_argument(
    ...
    parser.add_argument("--output_dir", type=str, default='chinese-macbert-large-squad_plot', help="Where to store the final model.")
...
def main():
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # pred_start = torch.argmax(outputs.start_logits)
            # pred_end = torch.argmax(outputs.end_logits)
            # gt_start = batch['start_positions'][0]
            # gt_end = batch['end_positions'][0]
            # print(pred_start, pred_end)
            # print(gt_start, gt_end)
            start = torch.argmax(outputs.start_logits) == batch['start_positions'][0]
            end = torch.argmax(outputs.end_logits) == batch['end_positions'][0]

            correct.append(start and end)
            losses.append(loss.item())
            if len(correct) % 200 == 0:
                accuracy = (sum(correct) / len(correct)).item()
                l = sum(losses) / len(losses)
                print(f'accuracy = {accuracy}, loss = {l}')

                tf_logger.add_scalar('EM', accuracy, len(correct))
                tf_logger.add_scalar('loss', l, len(correct))
    ...
    if args.output_dir is not None:
        ...
        print(args.model_name_or_path[-1])
        print(l)
        print(eval_metric)
        f = open('plot.csv', 'a')
        writer = csv.writer(f)
        writer.writerow([args.model_name_or_path[-1], l, eval_metric['exact_match']])
        f.close()
